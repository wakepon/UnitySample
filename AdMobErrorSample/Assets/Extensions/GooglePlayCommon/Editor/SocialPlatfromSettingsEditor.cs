...
public class SocialPlatfromSettingsEditor : Editor {
	...
	public static void UpdatePluginDefines() {
		...
		if(PluginsInstalationUtil.IsFacebookInstalled) {
			int version = SA.Config.FB_SDK_MajorVersionCode;
			if(version != 0) {
				if(version >= 7) {
	...
	public static void UpdateManifest() {
		...
		if(PluginsInstalationUtil.IsFacebookInstalled) {
			...
			if (SA.Config.FB_SDK_MajorVersionCode >= 7) {
	...
	public static void ResetSettings() {
		SA.Util.Files.DeleteFile("Extensions/GooglePlayCommon/Resources/SocialSettings.asset");
	...
	public static void UpdateVersionInfo() {
		SA.Util.Files.Write(SA_VersionsManager.MSP_VERSION_INFO_PATH, SocialPlatfromSettings.VERSION_NUMBER);
		UpdateManifest();
	}
